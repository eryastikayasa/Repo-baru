...
static void oled_send_cmd(uint8_t cmd)
{
    ...
    i2c_master_transmit(oled_dev, b, sizeof(b), pdMS_TO_TICKS(100));
}
...
static void oled_clear(void)
{
    ...
    for (uint8_t page = 0; page < 8; ++page) {
        ...
        oled_send_cmd(0x00);
        oled_send_cmd(0x10);
        i2c_master_transmit(oled_dev, tx, sizeof(tx), pdMS_TO_TICKS(100));
    }
}
...
void oled_init(void)
{
    ...
    bus_config.sda_io_num = OLED_SDA_PIN;
    bus_config.scl_io_num = OLED_SCL_PIN;
    ...
    err = i2c_master_bus_add_device(i2c_bus, &dev_config, &oled_dev);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Gagal menambahkan device OLED: %s", esp_err_to_name(err));
        oled_dev = NULL;
        return;
    }
    ...
    oled_ready = true;
    ESP_LOGI(TAG, "OLED SSD1306 siap: SDA=%d SCL=%d ADDR=0x%02X",
...
void display_status(const char* text)
{
    ...
    oled_clear();
    if (text && text[0] != '\0') {
        // Status text is intentionally kept in the log for now. The face
        // renderer is the primary UI; this avoids a second large font table.
        current_face_state = FACE_IDLE;
    }
    xSemaphoreGive(oled_mutex);
}

static void face_clear_buffer(void)
{
...
static void face_pixel(int x, int y)
{
    ...
    face_buffer[x + (y >> 3) * 128] |= (uint8_t)(1U << (y & 7));
}
...
static void face_ellipse(int cx, int cy, int rx, int ry)
{
    for (int y = -ry; y <= ry; ++y) {
        for (int x = -rx; x <= rx; ++x) {
            int lhs = x * x * ry * ry + y * y * rx * rx;
            int rhs = rx * rx * ry * ry;
            if (lhs <= rhs && (lhs > rhs - (rx + ry) * 3 || current_face_state != FACE_IDLE))
...
{
    if (!oled_ready) oled_init();
    if (!oled_ready || !oled_mutex || !oled_dev) return;
    ...
    if (current_face_state == FACE_SLEEP) {
        ...
        if (current_face_state == FACE_SPEAKING) {
            ...
            face_line(53, 51, 59, 54); face_line(59, 54, 69, 54); face_line(69, 54, 75, 51);
        } else if (current_face_state == FACE_SAD) {
            face_line(55, 55, 64, 51); face_line(64, 51, 73, 55);
        } else if (current_face_state == FACE_ERROR) {
            face_line(56, 51, 72, 57); face_line(72, 51, 56, 57);
        }
    }

    xSemaphoreTake(oled_mutex, portMAX_DELAY);
    for (uint8_t page = 0; page < 8; ++page) {
        uint8_t c1[2] = {0x00, (uint8_t)(0xB0 + page)};
        uint8_t c2[2] = {0x00, 0x00};
        uint8_t c3[2] = {0x00, 0x10};
        uint8_t tx[129];
        tx[0] = 0x40;
        memcpy(&tx[1], &face_buffer[page * 128], 128);
        i2c_master_transmit(oled_dev, c1, sizeof(c1), pdMS_TO_TICKS(100));
        i2c_master_transmit(oled_dev, c2, sizeof(c2), pdMS_TO_TICKS(100));
        i2c_master_transmit(oled_dev, c3, sizeof(c3), pdMS_TO_TICKS(100));
        i2c_master_transmit(oled_dev, tx, sizeof(tx), pdMS_TO_TICKS(100));
    }
    xSemaphoreGive(oled_mutex);
}
